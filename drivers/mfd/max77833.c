...
static int max77833_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *dev_id)
{
	...
	//mutex_init(&max77833->i2c_lock); unused it seems
	...
	if (!max77833->muic) {
               dev_err(max77833->dev, "Failed to allocate I2C device for MUIC\n");
               return -ENODEV;
        }
	i2c_set_clientdata(max77833->muic, max77833);

	max77833->fuelgauge = i2c_new_dummy(i2c->adapter, I2C_ADDR_FG);
	if (!max77833->fuelgauge) {
                dev_err(max77833->dev, "Failed to allocate I2C device for Fuelgauge\n");
                ret = -ENODEV;
                goto err;
	}
